(function () {
  ...
  // Модалка в спекции Автор

  const modalAutor = document.querySelector('.modal-autor');
  const modalButtonAutor = document.querySelectorAll('.autor__name');

  modalButtonAutor.forEach(btn =>
    btn.addEventListener('click', e => {
      e.preventDefault();
  ...
  modalAutor.addEventListener('click', e => {
    if (e.target.closest('.modal-autor__cancel') || e.target.classList.contains('modal-autor')) {
      e.preventDefault();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const button = document.querySelector('.button__up');
    if (!button) return;
